...
export function CreateGroupModal({ onClose, onCreated }: Props) {
  ...
  useEffect(() => {
    ...
    const applyUsers = (apiUsers: UserDto[]) => {
      setAllUsers(
        collectAllContacts(
          user.userId,
          apiUsers,
          storeUsers,
          recentChats.map(userFromRecentChat)
        )
      );
    };
  ...
  const selectedUsers = useMemo(
    ...
    () => allUsers.filter((u) => [...selected].some((id) => sameUserId(id, u.id))),
  ...
  const toggle = (id: string) => {
    ...
    setSelected((prev) => {
      ...
      const existing = [...next].find((entry) => sameUserId(entry, id));
              ...
              <div className="member-multiselect-dropdown">
                ...
                <div className="member-multiselect-options" role="listbox" aria-multiselectable="true">
                  ...
                  {loadingUsers && (
                    ...
                    <p className="modal-empty-hint">Loading users…</p>
                  ...
                  {!loadingUsers && allUsers.length > 0 && filteredUsers.length === 0 && (
                  ...
                  {filteredUsers.map((u) => {
                    ...
                    const isSelected = [...selected].some((id) => sameUserId(id, u.id));
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`member-option-status ${u.isOnline ? 'online' : ''}`}>
